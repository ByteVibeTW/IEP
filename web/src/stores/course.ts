...
export const useCourseStore = defineStore('courseStore', {
    ...
    courses: [
      {
        ...
        course_type: '程式設計',
        course_intro:
          '從零開始學習 Vue.js 前端框架，包含組件開發、狀態管理、路由配置等核心概念。適合有基礎 HTML/CSS/JavaScript 知識的學習者。',
        course_outline:
          '1. Vue.js 基礎概念\n2. 組件開發與通信\n3. Vuex 狀態管理\n4. Vue Router 路由配置\n5. 實戰專案開發\n6. 部署與優化',
        course_price: 2999,
        course_image: '',
      ...
      },
      {
        ...
        course_type: '程式設計',
        course_intro:
          '學習使用 Spring Boot 框架開發 RESTful API，包含資料庫操作、安全認證、微服務架構等進階主題。',
        course_outline:
  ...
  actions: {
    ...
    getMyCourses(user_id: string) {
      this.myCourses = this.courses.filter((course: Course) => course.students.includes(user_id))
    },
    saveCurrentClass(course_id: string) {
      this.currentClass = this.courses.find((course: Course) => course.course_id === course_id) || null
    },
    addGeneratedCourse(courseData: GeneratedCourseData) {
      // 添加 AI 生成的課程到課程列表
